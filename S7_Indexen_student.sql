...
-- VOOR
		-- Gather  (cost=1000.00..6151.27 rows=1000 width=96)
		--   Workers Planned: 2
		--     ->  Parallel Seq Scan on order_lines  (cost=0.00..5051.27 rows=417 width=96)
		-- 	        Filter: (stock_item_id = 9)
-- NA
		-- Bitmap Heap Scan on order_lines  (cost=12.04..2290.95 rows=1000 width=96)
		--   Recheck Cond: (stock_item_id = 9)
		--     ->  Bitmap Index Scan on ord_lines_si_id_idx  (cost=0.00..11.79 rows=1000 width=0)
