	...
	VALUES (8000, 'DONK', 'A', 'MANAGER', 7839, '1997-12-22', 2300, 50);
	...
	--A
	CREATE SEQUENCE afdelingsnr_sequence
	START 50
	INCREMENT 10;
	...
	VALUES
	(nextval('afdelingen_sequence'), 'ADMINISTRATIE', 'ROTTERDAM', 8000),
	(nextval('afdelingen_sequence'), 'HUMANRESOURCES', 'AMSTERDAM', 8000),
	(nextval('afdelingen_sequence'), 'KLANTENSERVICE', 'ARNHEM', 8000);

	--C

	ALTER TABLE afdelingen
	ALTER COLUMN anr SET DATA TYPE CHAR(3);
	...
	CREATE TABLE adressen (
		postcode CHAR(6) CHECK (postcode ~ '^[A-Z]{4}[0-9]{2}$') PRIMARY KEY,
		huisnummer CHAR(10) PRIMARY KEY,
		ingangsdatum DATE,
		einddatum DATE,
		telefoon NUMERIC(10, 0) UNIQUE,
		FOREIGN KEY (anr) REFERENCES afdelingen(anr)
	);
	...
	ADD CONSTRAINT medewerker_comm_cnstr
	CHECK (
	    (functie = 'VERKOPER' AND comm IS NOT NULL) OR
	    (function <> 'VERKOPER' AND comm IS NULL)
	);
